...
### Validation files and controlled algorithm follow-up

The two additional independent-validation reports have zero failed checks. Their embedded route policies identify direct-enabled and warehouse-only respectively, independent of attachment order. All nine scenario service levels are1.0; maximum supply-balance residual is3.7253e-9 tonnes and inventory-balance residual1.8626e-9 tonnes. Candidate activation, existing activation, bounds, integrality and frozen-network checks pass. Cost-reconstruction maximum absolute residuals are332 and248 monetary units respectively, accepted under the existing relative and sparse-export rounding budgets; these are not physical balance residuals. No tolerance was relaxed.

Re-serializing each parsed attachment with the exporter's sorted-key, indented UTF-8/LF JSON convention reproduces the supplied receipt hash: direct-enabled `ee34de00c176482e2f11876634bd2e566b5f81939325ae635535753db1857103`, warehouse-only `ed07d5cb7ea58163137644f0c9eb3ec28cc318d6291f7297bc29b65c31a926fb`. Their associated reported run identities are `e8077a4271f15ea2b72e1c08905950eae9a6d30bb6ecb51d8aecdc3e9be6bfd8` and `e24e0f5108e69abc5419b5dee78aa2182b5d468c9d756de9554dc0583069b23b`. This checks the supplied validation content against the receipt, not all original HPC artifact bytes locally. The prior on-cluster campaign audit remains the evidence for full completion-identity verification.

The supplied receipts list identical hashes for warehouse decisions, investment saturation and interhub connectivity products across the pair. This is consistent with the common interhub inputs and an incompletely optimized hierarchy; it does not establish optimal investment equivalence. The full selected-network hashes differ as expected when direct arcs are included. Empty unmet-demand and EVPI/VSS exports are consistent with zero shortfall and disabled postoptimality, not evidence of missing positive demand or computed zero value-of-information metrics.

The mathematical contract explicitly measures reception slack in overflow tonnes per period and static slack as stock tonnes at a warehouse-period, with an equal-weight capacity diagnostic. It does not claim equivalence to the historical shared-slack penalty. Preserve this distinction in reporting and do not reinterpret either slack as installed capacity.

The new `prepare_h400_dual_sensitivity.py` creates a separate, hash-verified clone of the original campaign with Method1 as the sole solver-parameter change. It does not rebuild distances, change constraints, increase memory/time allowances, or modify optimizer source files. The first bounded diagnostic is direct-enabled index1 only; index0 remains unsubmitted. Dual simplex has an official memory-use rationale relative to concurrent LP methods but no measured improvement for this400-hub case yet. See [the sensitivity runbook](nine_h400_dual_sensitivity.md). Local regression tests verify isolation, unchanged settings, checksums and refusal to overwrite or use confounded baselines.
