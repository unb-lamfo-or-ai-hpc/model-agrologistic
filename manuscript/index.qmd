...
abstract: >-
  ...
  Road data have been materialized for 215 and 500 warehouses; the selected
  215-warehouse nine-scenario models contain approximately 14 million variables.
  Nine of ten reference executions are accepted at this working draft's cutoff;
  the remaining HPC acceptance is pending. The contribution concerns auditable
  computational experimentation, not a new distributed MILP algorithm. Exact
  historical numerical replication, measured parallel speedup and a complete
  scalability frontier are not established.
keywords:
  - grain storage
  - facility location
...
We credit that work for the grain-allocation and storage-planning foundation;
the present study does not introduce the underlying application model as new.
Its bounded reconstruction is distinguished from policy extensions with separate
slacks, service priorities and expanded candidate data. The published review
...
|:--|:--|:--|
| Agricultural MILP solution strategy | @shastri2011 | Extensive-form Gurobi baseline; decomposition is future work |
...
The gap addressed here is operational: connecting an agricultural investment
...
At the current cutoff, road-data preparation covers 215 and 500 warehouses,
whereas the selected policy optimization evidence concerns 215 warehouses.
Nine of ten reference executions are accepted, with the final warehouse-only
nine-scenario execution unresolved. The manuscript does not yet demonstrate
a new solution algorithm, parallel speedup or a complete scalability frontier.
Final comparative tables require matching frozen exports. An unsuccessful
last retry will be retained as a bounded computational limitation, not
suppressed or converted into acceptance.
