...
abstract: >-
  ...
  Nine of ten reference executions satisfied the validation protocol. The final
  warehouse-only nine-scenario trial achieved zero domestic shortage but reached
  the 14,400-second optimization limit with a 100% capacity-pass gap; the
  economic pass was not executed. The contribution concerns auditable
...
Road-data preparation covered 215 and 500 warehouses, whereas the selected
policy optimization experiments concerned 215 warehouses. Nine of ten reference
executions satisfied the protocol. The final warehouse-only nine-scenario trial
achieved domestic service but reached the optimization time limit with a 100%
capacity-pass gap and no economic pass. This negative result delineates the
observed computational boundary without establishing a complete scalability
frontier. No new solution algorithm or measured parallel speedup is claimed.
The bounded experiment provides a reproducible baseline for decomposition,
controlled resource comparisons and larger candidate populations.
...
The committed manuscript data distinguish final validation outcomes, routing
counts and scheduler observations from unavailable comparative optimization
exports. Private runtime credentials and local library attachments are excluded.
